...
function BoardPrize() {
  ...
  const CreatePrize = async () => {
    try {
      ...
      fetch()
      setMessage({content: "Tạo giải thưởng thành công",open: true,severity:"success"})
      setOpen(false)
    }
    catch(e) {
      setMessage({content: "bị lỗi, hãy thử lại sau",open: true, severity:"error"})
  ...
  return (
    <div className='board prize'>
      <Box display="flex" gap={2} flexDirection='row' alignItems="center"  justifyContent="space-between" >
      ...
      <Box display="flex" gap={2} flexDirection='row' alignItems="center" justifyContent="space-between" width="80%" >
        <p>Tên giải thưởng</p>
        <p>Link ảnh</p>
      </Box>
        {
        listPrize?.map((item,index)=>(
          <Box display="flex" gap={2} flexDirection='row' alignItems="center" key={index} >
            <TextField
              hiddenLabel
            ...
            />
            <Button variant="contained" onClick={()=>onClickUpdate(item.id, item.name, item.imgUrl, index)}>Cập nhật</Button>
            <Button variant="contained" onClick={()=>onDelete(item.id)} >Xóa</Button>
          </Box>
        ))
        }
